...
const USER_REGEX = /^[a-zA-Z]+\.[a-zA-Z0-9]{1,17}$/;
  ...
  const loginHandler = async (event) => {
    ...
    try {
      ...
      if (resp.data.data.role === "Patron") {
        ...
        setIsLoading(false);
        Navigate("/patron-dashboard");
      } else if (resp.data.data.role === "HOD") {
        localStorage.setItem("HOD", resp.data.token);
        toast.success("Login successfull");
        await delay(2000);
        setIsLoading(false);
        Navigate("/hod-dashboard");
      } else if (resp.data.data.role === "Dean") {
        localStorage.setItem("Dean", resp.data.token);
        toast.success("Login successfull");
        await delay(2000);
        setIsLoading(false);
        Navigate("/dean-dashboard");
